...
describe SendRegard do
  context "#call" do
    ...
    let(:current_user) { create :user, email: "example2@o2.pl", password: "password" }


    it "should have required arguments" do
      regard_mailer = double("RegardMailer")
      expect(RegardMailer).to receive(:send_regard).with( current_user_email: current_user.email,
                                         user_email: user.email ) { regard_mailer }
